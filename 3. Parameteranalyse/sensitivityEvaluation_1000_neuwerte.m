function [EvalResult,Info] = sensitivityEvaluation_1000_neuewerte(p)
...
%
% Auto-generated by SSATOOL on 30-Sep-2022 12:56:53.
%
...
if nargin < 1 || isempty(p)
    ...
    p(2).Minimum = 0.01;
    p(2).Maximum = 1;
    p(3).Value = 0.7999999999999999;
    p(3).Minimum = 0.1;
    p(3).Maximum = 1.5;
    p(4).Value = 0.7999999999999999;
    p(4).Minimum = 0.1;
    p(4).Maximum = 1.5;
    p(5).Value = 1.05;
    p(5).Minimum = 0.1;
...
phi_punkt_punkt_settling_time_Data.p = P;
phi_punkt_punkt_settling_time_Data.Nominal.phi_punkt_punkt = phi_punkt_punkt_Log.Values;
Vals.phi_punkt_punkt_settling_time = SettlingTimeFilter(phi_punkt_punkt_settling_time_Data);
validateattributes(Vals.phi_punkt_punkt_settling_time,{'double'},{'real','vector'},'','Vals.phi_punkt_punkt_settling_time');
thetaSettlingTime_Data.p = P;
thetaSettlingTime_Data.Nominal.theta = theta_Log.Values;
Vals.thetaSettlingTime = SettlingTimeFilter(thetaSettlingTime_Data);
validateattributes(Vals.thetaSettlingTime,{'double'},{'real','vector'},'','Vals.thetaSettlingTime');
...
Vals.phi_punkt_punkt_settling_time = max(Vals.phi_punkt_punkt_settling_time(:));
Vals.thetaSettlingTime = max(Vals.thetaSettlingTime(:));
